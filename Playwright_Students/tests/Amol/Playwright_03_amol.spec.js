const { test, expect } = require("@playwright/test") // suggession 
const { waitForDebugger } = require("inspector")
const fruitsdate = require("../../Fixture/TestData/Testdata01.json")
...
test("letcode:- fruits checking functionality of let code for dropdown", async ({ browser }) => {
    ...
    // go to website
    await page.goto(fruitsdate.Mydata[2].LetCodeURL)
    // Verify the title of Webpage
    await expect(page).toHaveTitle(fruitsdate.Mydata[2].Title)
    ...
    // Select the header of fruit select
    await page.locator('[class="control"] [class="select"] select[id="fruits"]').selectOption(fruitsdate.Mydata[2].Selectefruits)
    // Verify the header of fruit select
    await expect(page.locator('[class="control"] [class="select"] select[id="fruits"]')).toHaveValue(fruitsdate.Mydata[2].selectedValue)

    // Select the Fruits from dropdown:- Apple
    await page.locator('[class="control"] [class="select"] select[id="fruits"]').selectOption(fruitsdate.Mydata[2].selectApple)
    // Verify the Fruits from dropdown:- Apple
    await expect(page.locator('[class="control"] [class="select"] select[id="fruits"]')).toHaveValue(fruitsdate.Mydata[2].applevalue)
    // select the selected Fruits
    await page.locator('[class="subtitle"]').textContent()
    // Verify the selected Fruits
    await expect(page.locator('[class="subtitle"]')).toContainText(fruitsdate.Mydata[2].aplleverify)

    // Select the Fruits from dropdown:- Mango
    await page.locator('[class="control"] [class="select"] select[id="fruits"]').selectOption(fruitsdate.Mydata[2].selectMango)
    // Verify the Fruits from dropdown:- Mango
    await expect(page.locator('[class="control"] [class="select"] select[id="fruits"]')).toHaveValue(fruitsdate.Mydata[2].Mangovalue)
    // select the selected Fruits
    await page.locator('[class="subtitle"]').textContent()
    // Verify the selected Fruits
    await expect(page.locator('[class="subtitle"]')).toContainText(fruitsdate.Mydata[2].Mangoverify)

    // Select the Fruits from dropdown:- Orange
    await page.locator('[class="control"] [class="select"] select[id="fruits"]').selectOption(fruitsdate.Mydata[2].selectOrange)
    // Verify the Fruits from dropdown:- Orange
    await expect(page.locator('[class="control"] [class="select"] select[id="fruits"]')).toHaveValue(fruitsdate.Mydata[2].Orangevalue)
    // select the selected Fruits
    await page.locator('[class="subtitle"]').textContent()
    // Verify the selected Fruits
    await expect(page.locator('[class="subtitle"]')).toContainText(fruitsdate.Mydata[2].Orangeverify)

    // Select the Fruits from dropdown:- Banana
    await page.locator('[class="control"] [class="select"] select[id="fruits"]').selectOption(fruitsdate.Mydata[2].selectBanana)
    // Verify the Fruits from dropdown:- Banana
    await expect(page.locator('[class="control"] [class="select"] select[id="fruits"]')).toHaveValue(fruitsdate.Mydata[2].Bananavalue)
    // select the selected Fruits
    await page.locator('[class="subtitle"]').textContent()
    // Verify the selected Fruits
    await expect(page.locator('[class="subtitle"]')).toContainText(fruitsdate.Mydata[2].Bananaverify)

    // Select the Fruits from dropdown:- Pine Apple
    await page.locator('[class="control"] [class="select"] select[id="fruits"]').selectOption(fruitsdate.Mydata[2].selectPineApple)
    // Verify the Fruits from dropdown:- Pine Apple
    await expect(page.locator('[class="control"] [class="select"] select[id="fruits"]')).toHaveValue(fruitsdate.Mydata[2].PineApplevalue)
    // select the selected Fruits
    await page.locator('[class="subtitle"]').textContent()
    // Verify the selected Fruits
    await expect(page.locator('[class="subtitle"]')).toContainText(fruitsdate.Mydata[2].PineAppleverify)
